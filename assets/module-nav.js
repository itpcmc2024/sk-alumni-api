(()=>{
  ...
  function enhance(){
    try{
      ...
      if(!document.querySelector('[data-sk-nav="v2.6.41"]')){const old=document.querySelector('header.top');const holder=document.createElement('div');holder.innerHTML=header();const fresh=holder.firstElementChild;if(old&&fresh)old.replaceWith(fresh);else if(fresh)document.body.insertAdjacentElement('afterbegin',fresh)}
      addAdminActions();
