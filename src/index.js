...
export default {
  async fetch(request,env){
    ...
    let sql=null;
    try{
      if(path==="/") return json(request,{success:true,app:"SK Alumni API",version:"2.6.75",status:"online"});
      sql=db(env);
      if(path==="/api/health"&&request.method==="GET"){
        const r=await sql`SELECT current_database() database,NOW() server_time`;
        return json(request,{success:true,service:"sk-alumni-api",database:r[0].database,server_time:r[0].server_time,version:"2.6.75"});
      }
      ...
      if(path==="/api/settings/public"&&request.method==="GET"){
        ...
        const data={};for(const r of rows)data[r.setting_key]=r.setting_value;data.APP_VERSION='V2.6.75';
      ...
      if(path==="/api/admin/auth-check"&&request.method==="GET"){
        ...
        return json(request,{success:true,authorized:true,version:"2.6.75"});
      }
      ...
      if(/^\/api\/admin\/payments\/[^/]+\/receipt$/.test(path)&&request.method==="GET"){
        ...
        await sql`ALTER TABLE payments ADD COLUMN IF NOT EXISTS receipt_no TEXT`;
        await sql`ALTER TABLE payments ADD COLUMN IF NOT EXISTS receipt_issued_at TIMESTAMPTZ`;
        /* V2.6.75: do not fabricate receipt numbers; official numbers come from receipt books. */
      ...
      if(path==="/api/admin/receipts"&&request.method==="GET"){
        const denied=await requireAdmin(request,env,sql);if(denied)return denied;await ensureV2616Schema(sql);
        /* V2.6.75: do not fabricate receipt numbers; official numbers come from receipt books. */
        /* V2.6.75: official donation receipt numbers come from receipt books. */
      ...
      if(path==="/api/admin/settings"&&request.method==="PUT"){
        ...
        await sql`INSERT INTO app_settings(setting_key,setting_value,updated_at) VALUES('APP_VERSION','V2.6.75',NOW()) ON CONFLICT(setting_key) DO UPDATE SET setting_value=EXCLUDED.setting_value,updated_at=NOW()`;
