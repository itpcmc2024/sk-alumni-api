...
function linePortalUrl(token,extra={}){
  const q=new URLSearchParams({line_token:token,from:'line',v:'2.7.02'});
...
async function recoverLineAdminMessages(sql){
  // V2.7.02: recover member-to-Admin text from the general LINE event log.
...
function lineBackground(ctx,promise){
  ...
  if(ctx&&typeof ctx.waitUntil==='function'){
    ...
    // V2.7.02 closed the shared PG client in fetch.finally() while waitUntil() jobs were still writing,
...
async function handleLineEvent(event,env,sql,ctx){
  ...
  // V2.7.02: LINE doesn't expose sticker image bytes, but webhook sticker metadata
  ...
  // V2.7.02: media messages enter the Admin Inbox quietly, like a natural LINE OA chat.
  ...
  if(t==='หน้าหลัก'){
    await lineReply(env,event.replyToken,lineLinkFlex('หน้าหลัก','เปิดหน้าแรกระบบสมาชิกสมาคมศิษย์เก่า',lineWebBase()+'index.html?v=2.7.02','เปิดหน้าหลัก'));
    lineBackground(ctx,saveLineEventNonCritical(event,sql,env));return;
  }
  if(t==='สนับสนุนรายปี'){
    await lineReply(env,event.replyToken,lineLinkFlex('สนับสนุนรายปี','แจ้งชำระค่าบำรุงสมาคมศิษย์เก่าฯ รายปี',lineWebBase()+'payment.html?v=2.7.02','เปิดหน้าแจ้งชำระ'));
    lineBackground(ctx,saveLineEventNonCritical(event,sql,env));return;
  }
  if(t==='บริจาค'){
    await lineReply(env,event.replyToken,lineLinkFlex('บริจาค','ร่วมสนับสนุนกิจกรรมและโครงการของสมาคม',lineWebBase()+'donation.html?v=2.7.02','เปิดหน้าบริจาค'));
    lineBackground(ctx,saveLineEventNonCritical(event,sql,env));return;
  }
  if(t==='โทรหาแอดมิน'||t==='ติดต่อแอดมิน'){
    let phone='',phoneEnabled=false;try{const rows=await sql`SELECT setting_key,setting_value FROM app_settings WHERE setting_key IN ('CONTACT_PHONE','CONTACT_PHONE_ENABLED')`;phone=clean(rows.find(x=>x.setting_key==='CONTACT_PHONE')?.setting_value).replace(/[^\d+]/g,'');phoneEnabled=clean(rows.find(x=>x.setting_key==='CONTACT_PHONE_ENABLED')?.setting_value)!=='false'}catch{}
  ...
  if(t==='ลงทะเบียน'||t.includes('ลงทะเบียน')){
    await lineReply(env,event.replyToken,lineLinkFlex('ลงทะเบียนสมาชิก','หลังได้รับรหัสสมาชิกแล้ว กลับมาพิมพ์ “เชื่อมบัญชี”',lineWebBase()+'register.html?v=2.7.02','เปิดหน้าลงทะเบียน'));
    lineBackground(ctx,saveLineEventNonCritical(event,sql,env));
    return;
  }
  if(['ตรวจสอบสถานะ','สมาชิก','สถานะสมาชิก','ตรวจสอบสมาชิก'].includes(t)||t.startsWith('สมาชิก ')){
    await lineReply(env,event.replyToken,lineLinkFlex('ตรวจสอบสถานะสมาชิก','ค้นหาและตรวจสอบสถานะสมาชิก',lineWebBase()+'status.html?v=2.7.02','ตรวจสอบสถานะ'));
    lineBackground(ctx,saveLineEventNonCritical(event,sql,env));
    return;
  }
  if(t==='สิทธิประโยชน์'||t==='สิทธิ'||t.includes('สิทธิประโยชน์')){
    await lineReply(env,event.replyToken,lineLinkFlex('สิทธิประโยชน์','ดูสิทธิประโยชน์สำหรับสมาชิกที่กำลังเปิดใช้งาน',lineWebBase()+'benefits.html?v=2.7.02','ดูสิทธิประโยชน์'));
    lineBackground(ctx,saveLineEventNonCritical(event,sql,env));
    return;
  }
  ...
  if(t==='เชื่อมบัญชี'||t==='ผูกบัญชี'||t==='เชื่อมสมาชิก'){
    ...
    try{
      ...
      await lineReply(env,event.replyToken,lineLinkFlex('เชื่อมบัญชีสมาชิก','ลิงก์นี้ใช้ได้ 15 นาที และหลังเชื่อมสำเร็จจะใช้ซ้ำไม่ได้',lineWebBase()+'line-link.html?token='+encodeURIComponent(token)+'&v=2.7.02','เชื่อมบัญชี'));
    }catch(err){
  ...
  // V2.7.02: quiet conversation mode. Ordinary member chat is stored without an automatic confirmation bubble.
...
export default {
  async fetch(request,env,ctx){
    ...
    let sql=null;
    try{
      if(path==="/") return json(request,{success:true,app:"SK Alumni API",version:"2.7.02",status:"online",line_webhook:"/api/line/webhook"});
      if(path==="/api/line/health"&&request.method==="GET"){
        return json(request,{success:true,version:"2.7.02",webhook:"/api/line/webhook",channel_secret_configured:!!env.LINE_CHANNEL_SECRET,access_token_configured:!!env.LINE_CHANNEL_ACCESS_TOKEN});
      }
      ...
      if(path==="/api/admin/line/messages"&&request.method==="GET"){
        ...
        // V2.7.02: durable Inbox + direct LINE-event fallback.
      ...
      if(path==="/api/health"&&request.method==="GET"){
        ...
        return json(request,{success:true,service:"sk-alumni-api",database:r[0].database,server_time:r[0].server_time,version:"2.7.02"});
      }
      ...
      if(path==="/api/settings/public"&&request.method==="GET"){
        ...
        const data={};for(const r of rows)data[r.setting_key]=r.setting_value;data.APP_VERSION='V2.7.02';
      ...
      if(path==="/api/admin/auth-check"&&request.method==="GET"){
        const denied=await requireAdmin(request,env,sql);if(denied)return denied;
        const a=await resolveAdmin(request,env,sql);return json(request,{success:true,authorized:true,version:"2.7.02",admin:a});
      }
      ...
      if(/^\/api\/admin\/accounts\/[^/]+$/.test(path)&&request.method==="PUT"){
        const denied=await requireOwner(request,env,sql);if(denied)return denied;await ensureAdminAccountsSchema(sql);
        const aid=decodeURIComponent(path.split('/').pop()).toUpperCase(),b=await body(request),name=clean(b.full_name),role=clean(b.role)==='owner'?'owner':'admin',active=b.active!==false,key=clean(b.admin_key),newAid=clean(b.new_admin_id).toUpperCase()||aid;
        if(!newAid||!name)return json(request,{success:false,message:'กรุณากรอก User ID และชื่อ-นามสกุล'},400);
        if(key&&key.length<8)return json(request,{success:false,message:'Admin Key ใหม่ต้องมีอย่างน้อย 8 ตัวอักษร'},400);
      ...
      if(path==="/api/admin/settings"&&request.method==="PUT"){
        ...
        await sql`INSERT INTO app_settings(setting_key,setting_value,updated_at) VALUES('APP_VERSION','V2.7.02',NOW()) ON CONFLICT(setting_key) DO UPDATE SET setting_value=EXCLUDED.setting_value,updated_at=NOW()`;
