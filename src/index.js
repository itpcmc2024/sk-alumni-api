...
async function handleLineEvent(event,env,sql){
  ...
  // Logging must never block a LINE reply. If DB/logging is temporarily unavailable,
  // continue routing the user's command.
  if(sql){
    try{
      ...
      console.error('LINE log error (non-blocking)',err);
    }
  }
  ...
  // BASIC COMMANDS: intentionally no member-link database lookup before replying.
  ...
  if(t==='เชื่อมบัญชี'||t==='ผูกบัญชี'||t==='เชื่อมสมาชิก'){
    ...
    if(!sql){
      await lineReply(env,event.replyToken,{type:'text',text:'ระบบเชื่อมบัญชีกำลังไม่พร้อมใช้งานชั่วคราว กรุณาลองใหม่อีกครั้ง'});
      return;
    }
    try{
  ...
  if(['ข้อมูลของฉัน','ข้อมูลสมาชิก','บัญชีสมาชิก'].includes(t)){
    if(!sql||!userId){
      await lineReply(env,event.replyToken,{type:'text',text:'ระบบข้อมูลสมาชิกกำลังไม่พร้อมใช้งานชั่วคราว กรุณาลองใหม่อีกครั้ง'});
      return;
    }
    try{
      const linked=await linkedLineMember(sql,userId);
...
export default {
  async fetch(request,env){
    ...
    let sql=null;
    try{
      if(path==="/") return json(request,{success:true,app:"SK Alumni API",version:"2.6.79.1",status:"online",line_webhook:"/api/line/webhook"});
      ...
      if(path==="/api/health"&&request.method==="GET"){
        ...
        return json(request,{success:true,service:"sk-alumni-api",database:r[0].database,server_time:r[0].server_time,version:"2.6.79.1"});
      }
      ...
      if(path==="/api/settings/public"&&request.method==="GET"){
        ...
        const data={};for(const r of rows)data[r.setting_key]=r.setting_value;data.APP_VERSION='V2.6.79.1.1';
      ...
      if(path==="/api/line/link-account"&&request.method==="POST"){
        ...
        const occupied=await sql`SELECT line_user_id FROM line_users WHERE member_code=${code} AND line_user_id<>${tr[0].line_user_id} AND follow_status='active' LIMIT 1`;
        if(occupied.length)return json(request,{success:false,message:'รหัสสมาชิกนี้เชื่อมกับ LINE อื่นอยู่แล้ว กรุณาติดต่อ Admin หากต้องการเปลี่ยนบัญชี LINE'},409);
        await sql.begin(async tx=>{
          await tx`UPDATE line_users SET member_code=NULL,updated_at=NOW() WHERE line_user_id=${tr[0].line_user_id}`;
          await tx`UPDATE line_users SET member_code=${code},follow_status='active',last_seen_at=NOW(),updated_at=NOW() WHERE line_user_id=${tr[0].line_user_id}`;
          await tx`UPDATE members SET line_user_id=${tr[0].line_user_id},updated_at=NOW() WHERE member_code=${code}`;
      ...
      if(path==="/api/admin/auth-check"&&request.method==="GET"){
        ...
        return json(request,{success:true,authorized:true,version:"2.6.79.1"});
      }
      ...
      if(/^\/api\/admin\/payments\/[^/]+\/receipt$/.test(path)&&request.method==="GET"){
        ...
        await sql`ALTER TABLE payments ADD COLUMN IF NOT EXISTS receipt_no TEXT`;
        await sql`ALTER TABLE payments ADD COLUMN IF NOT EXISTS receipt_issued_at TIMESTAMPTZ`;
        /* V2.6.79.1.1: do not fabricate receipt numbers; official numbers come from receipt books. */
      ...
      if(path==="/api/admin/receipts"&&request.method==="GET"){
        const denied=await requireAdmin(request,env,sql);if(denied)return denied;await ensureV2616Schema(sql);
        /* V2.6.79.1.1: do not fabricate receipt numbers; official numbers come from receipt books. */
        /* V2.6.79.1.1: official donation receipt numbers come from receipt books. */
      ...
      if(path==="/api/admin/settings"&&request.method==="PUT"){
        ...
        await sql`INSERT INTO app_settings(setting_key,setting_value,updated_at) VALUES('APP_VERSION','V2.6.79.1.1',NOW()) ON CONFLICT(setting_key) DO UPDATE SET setting_value=EXCLUDED.setting_value,updated_at=NOW()`;
