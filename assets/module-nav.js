(()=>{
  ...
  const header=()=>`<header class="sk-module-nav" data-sk-nav="v2.6.23"><div class="sk-nav-wrap">
    <a class="sk-nav-brand" href="index.html?v=2.6.23"><img class="sk-nav-logo" src="assets/association-logo.png?v=2.6.23" alt="โลโก้สมาคม"><div><div class="sk-nav-title" data-app-name><span class="sk-app-line">ระบบสมาชิกสมาคมศิษย์เก่า</span><span class="sk-app-line">นูรุ้ลอิสลามสัมพันธ์ (สุเหร่าเขียว)</span></div><div class="sk-nav-sub">🌙 Nurul Islam · SK Alumni Member System</div></div></a>
    <nav class="sk-nav-icons">${items.map(x=>`<a class="${active(x[0])?'active':''}" href="${x[0]}?v=2.6.23"><span class="ico">${x[1]}</span>${x[2]}</a>`).join('')}</nav>
    <div class="sk-mobile-home"><a href="index.html?v=2.6.23">🏠 หน้าแรก</a>${path.startsWith('admin')&&path!=='admin-home.html'?'<a class="sk-mobile-admin-back" href="admin-home.html">← กลับศูนย์จัดการระบบ</a>':''}</div>
  </div></header>`;

  function enhance(){
    try{
      document.querySelectorAll('.v26-homebar,.v25-homebar,.homebtn').forEach(x=>x.remove());
      if(!document.querySelector('[data-sk-nav="v2.6.23"]')){
      ...
      if(path.startsWith('admin') && path!=='admin-home.html' && !document.querySelector('.sk-admin-back-inline')){
        ...
        if(main){const a=document.createElement('a');a.className='sk-admin-back-inline';a.href='admin-home.html?v=2.6.23';a.textContent='← กลับศูนย์จัดการระบบ';if(tabs){tabs.classList.add('sk-tabs-with-back');tabs.appendChild(a)}else main.insertAdjacentElement('afterbegin',a)}
      }
