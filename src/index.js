...
export default {
  async fetch(request,env){
    ...
    let sql=null;
    try{
      if(path==="/") return json(request,{success:true,app:"SK Alumni API",version:"2.6.79.2",status:"online",line_webhook:"/api/line/webhook"});
      if(path==="/api/line/health"&&request.method==="GET"){
        return json(request,{success:true,version:"2.6.79.2",webhook:"/api/line/webhook",channel_secret_configured:!!env.LINE_CHANNEL_SECRET,access_token_configured:!!env.LINE_CHANNEL_ACCESS_TOKEN});
      }
      ...
      if(path==="/api/health"&&request.method==="GET"){
        ...
        return json(request,{success:true,service:"sk-alumni-api",database:r[0].database,server_time:r[0].server_time,version:"2.6.79.2"});
      }
      ...
      if(path==="/api/settings/public"&&request.method==="GET"){
        ...
        const data={};for(const r of rows)data[r.setting_key]=r.setting_value;data.APP_VERSION='V2.6.79.2.1';
      ...
      if(path==="/api/admin/auth-check"&&request.method==="GET"){
        ...
        return json(request,{success:true,authorized:true,version:"2.6.79.2"});
      }
      ...
      if(/^\/api\/admin\/payments\/[^/]+\/receipt$/.test(path)&&request.method==="GET"){
        ...
        await sql`ALTER TABLE payments ADD COLUMN IF NOT EXISTS receipt_no TEXT`;
        await sql`ALTER TABLE payments ADD COLUMN IF NOT EXISTS receipt_issued_at TIMESTAMPTZ`;
        /* V2.6.79.2.1: do not fabricate receipt numbers; official numbers come from receipt books. */
      ...
      if(path==="/api/admin/receipts"&&request.method==="GET"){
        const denied=await requireAdmin(request,env,sql);if(denied)return denied;await ensureV2616Schema(sql);
        /* V2.6.79.2.1: do not fabricate receipt numbers; official numbers come from receipt books. */
        /* V2.6.79.2.1: official donation receipt numbers come from receipt books. */
      ...
      if(path==="/api/admin/settings"&&request.method==="PUT"){
        ...
        await sql`INSERT INTO app_settings(setting_key,setting_value,updated_at) VALUES('APP_VERSION','V2.6.79.2.1',NOW()) ON CONFLICT(setting_key) DO UPDATE SET setting_value=EXCLUDED.setting_value,updated_at=NOW()`;
